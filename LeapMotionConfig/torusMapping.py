...
###################################### ENCODING #######################################
###################################### ENCODING #######################################
############### MULTIJOINT CONVERSION TO SINGLE TORUS MAPPED COORDINATE ###############
###################################### ENCODING #######################################
###################################### ENCODING #######################################
...
def convert_multijoint_to_torus_space(single_motion_data, return_type='RESULTANT'):
    ...
    for i in range(row_shape):
        ...
        # proximal bone x/y coordinate: it can only rotate around a circle, so do that!
        ...
        #the mid x/y coordinates: since mid_base_x is in correspondance to a unit circle, add it with respect to the proximal joint.
        mid_x = mid_base_x + prox_x
        mid_y = mid_base_y + prox_y

        if return_type == 'RESULTANT':      conversion_dict[finger_dict_names[i]] = [mid_x , mid_y]
        elif return_type == 'PROXIMAL':     conversion_dict[finger_dict_names[i]] = [ prox_x , prox_y ]
        elif return_type == 'MIDDLE':       conversion_dict[finger_dict_names[i]] = [ mid_base_x , mid_base_y ]
...
###################################### DECODING #######################################
###################################### DECODING #######################################
############### SINGLE TORUS MAPPED COORDINATE TO MULTIJOINT CONVERSION ###############
###################################### DECODING #######################################
###################################### DECODING #######################################

### RETURN QUADRANT WHERE:
#       #
#   4   #   1
# #############
#   3   #   2
#       #
# is the cartesian coordinate to quadrant
# TODO: turns out convention is to have 2 be the upper left, change code to match this (little bit of a pain in the ass to do)
def get_quadrant(x, y):
    if   (x>=0) and (y>=0):     return 1
    elif (x>=0) and (y<=0):     return 2
    elif (x<=0) and (y<=0):     return 3
    elif (x<=0) and (y>=0):     return 4
#left-hand side quadrant gets a negative number. This is just to make get_rotation_direction cleaner.
def polarize_quadrant(quadrant):
    if (quadrant == 3) or (quadrant == 4):      return -quadrant
    else:                                       return quadrant
#determine which direction to rotate a vector given its current endpoints and destination endpoints.
#Follows CCW/CW convention: clockwise rotation is negative. counterclockwise rotation is positive.
...
#outputs: dictionary of new proximal/middle phalanx coordinate information:
...
def two_joint_decode_torus_space_mapping(current_coordinates, destination_coordinates, bone_lengths=None):
    if bone_lengths is None:    bone_lengths = { 'PROX': 1.0, 'MID': 0.5 }
    ...
    #shorter variable names for computation.
    c_x_v1, c_y_v1 = [ current_coordinates[0][0], current_coordinates[0][1] ] #current proximal vector 1 coordinates
    c_x_v2, c_y_v2 = [ current_coordinates[1][0], current_coordinates[1][1] ] #current middle   vector 2 coordinates
    d_x, d_y = [ destination_coordinates[0], destination_coordinates[1] ]     #destination coordinates
    ...
    # equidistance away from it), discard the result that would form an anatomical solution that makes no sense.
    ...
    # vector_angle_2D doesn't determine which way to rotate the vector, so call get_rotation_direction to determine which way to rotate
